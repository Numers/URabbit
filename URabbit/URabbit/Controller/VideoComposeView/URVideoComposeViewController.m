//
//  URVideoComposeViewController.m
//  URabbit
//
...
#import "URVideoComposeSuccessViewController.h"
...
@interface URVideoComposeViewController ()<UTPlaySubViewProtocol,URSelectViewProtocol,URDownloadAlertViewProtocl>
{
    Resource *resource;
    NSString *movieURL;
    NSString *audioURL;
    ...
    URPlayView *playView;
    URSelectView *selectView;
    URSegmentView *segmentView;
    CMTime pausedTime;
    float duration;
...
@implementation URVideoComposeViewController
...
- (void)viewDidLoad {
    ...
    selectView = [[URSelectView alloc] initWithFrame:CGRectMake(0,SCREEN_HEIGHT - [UIDevice safeAreaBottomHeight] - 106, SCREEN_WIDTH, 106)];
    selectView.delegate = self;
    [self.view addSubview:selectView];
    
    segmentView = [[URSegmentView alloc] init];
    ...
    playView = [[URPlayView alloc] init];
...
-(void)save
{
    ...
    if (audioURL) {
        ...
        [[URVideoManager shareManager] mergeMovie:movieURL withAudio:audioURL output:tempVideoPath completely:^{
            GPUImageOutput<GPUImageInput> *movieFilter = [[URImageHanderManager shareManager] filterWithFilterType:currentFilterType];
            NSString *videoCompeletelyPath = [AppUtils videoPathWithUniqueIndex:currentComposition.templateId identify:@"complete"];
            [[URVideoManager shareManager] filterMovieWithInputUrl:tempVideoPath outputUrl:videoCompeletelyPath videoSize:resource.videoSize filter:movieFilter completely:^(BOOL result) {
                if (result) {
        ...
        GPUImageOutput<GPUImageInput> *movieFilter = [[URImageHanderManager shareManager] filterWithFilterType:currentFilterType];
        NSString *videoCompeletelyPath = [AppUtils videoPathWithUniqueIndex:currentComposition.templateId identify:@"complete"];
        [[URVideoManager shareManager] filterMovieWithInputUrl:movieURL outputUrl:videoCompeletelyPath videoSize:resource.videoSize filter:movieFilter completely:^(BOOL result) {
            if (result) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [AppUtils hiddenGIFHud:self.view];
                    currentComposition.moviePath = videoCompeletelyPath;
...
//        URVideoComposeSuccessViewController *videoComposeSuccessVC = [[URVideoComposeSuccessViewController alloc] initWithVideoURL:[NSURL fileURLWithPath:videoPath] templateId:currentComposition.templateId];
...
-(void)splitImages
{
    [[URVideoManager shareManager] splitVideo:[NSURL fileURLWithPath:movieURL] fps:resource.fps splitCompleteBlock:^(BOOL success, NSMutableArray *splitimgs) {
        if (success) {
            dispatch_async(dispatch_get_main_queue(), ^{
...
-(void)setCurrentFilterType:(FilterType)type
{
    ...
    filter = [[URImageHanderManager shareManager] filterWithFilterType:type];
}
...
#pragma -mark URPlayViewProtocol
...
#pragma -mark URSelectViewProtocol
...
-(NSMutableArray *)requestMusicViewDataSource
{
    ...
    [filterList addObject:info];
    [[URVideoComposeNetworkAPIManager shareManager] requestRecommendMusicWithTemplateId:currentComposition.templateId Callback:^(NSNumber *statusCode, NSNumber *code, id data, id errorMsg) {
...
-(void)share{
    ...
    [[URUMShareManager shareManager] indirectShareVideo:[NSURL fileURLWithPath:currentComposition.moviePath]];
}
@end
