...
#import "URHomeViewController.h"
#import "URHomeRecommendView.h"
#import "URHomeCollectionFootReusableView.h"
#import "RecommendTemplate.h"

#import "WSLWaterFlowLayout.h"
#import "URHomeCollectionViewCell.h"
...
@interface URHomeViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,WSLWaterFlowLayoutDelegate,URHomeRecommendViewProtocl>
{
    URHomeRecommendView *homeRecommendView;
...
@implementation URHomeViewController
...
//    _filterButton.titleEdgeInsets = UIEdgeInsetsMake(0, -(_filterButton.frame.size.width - _filterButton.imageView.frame.size.width ), 0, 0);
    ...
    [_collectionView registerClass:[URHomeCollectionViewCell class] forCellWithReuseIdentifier:homeTemplateCollectionViewCellIdentify];
    [_collectionView registerClass:[URHomeRecommendView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:homeTemplateCollectionHeadViewIdentify];
    [_collectionView registerClass:[URHomeCollectionFootReusableView class] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:homeTemplateCollectionFootViewIdentify];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if ([[GeneralManager defaultManager] isAuditSucess]) {
        if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
            URHomeRecommendView *recommendView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:homeTemplateCollectionHeadViewIdentify forIndexPath:indexPath];
            if (recommendList.count > 0) {
            ...
            URHomeCollectionFootReusableView *footView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:homeTemplateCollectionFootViewIdentify forIndexPath:indexPath];
            return footView;
        }
    }else{
        URHomeCollectionFootReusableView *footView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:homeTemplateCollectionFootViewIdentify forIndexPath:indexPath];
...
{
    URHomeCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:homeTemplateCollectionViewCellIdentify forIndexPath:indexPath];
...
{
    URCategoryViewController *categoryVC = [[URCategoryViewController alloc] initWithItems:recommendList selectIndex:0];
    categoryVC.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:categoryVC animated:YES];
}

#pragma -mark URHomeRecommendViewProtocl
-(void)gotoCategoryViewWithIndex:(NSInteger)index
{
    ...
    URCategoryViewController *categoryVC = [[URCategoryViewController alloc] initWithItems:recommendList selectIndex:index];
    categoryVC.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:categoryVC animated:YES];
}
@end
