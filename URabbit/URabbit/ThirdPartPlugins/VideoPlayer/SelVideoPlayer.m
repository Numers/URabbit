...
-(void)setConfiguration:(SelPlayerConfiguration *)configuration
{
...
-(void)setMovieUrl:(NSURL *)url
{
    if (_playerConfiguration) {
        _playerConfiguration.sourceUrl = url;
        [self _setupPlayer];
...
- (void)fullScreenButtonAction
{
    if (!_isFullScreen) {
        if (self.frame.size.width > self.frame.size.height) {
            ...
            [self _videoZoomInWithDirection:UIInterfaceOrientationPortrait];
        }
